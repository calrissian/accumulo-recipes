...
public class OrIterator implements SortedKeyValueIterator<Key, Value> {
    ...
    static protected String getUID(Key key) {
        try {
            ...
            idx = sKey.lastIndexOf(NULL_BYTE);
    ...
    public boolean jump(Key jumpKey) throws IOException {
        ...
        for (TermSource ts : sources) {
            ...
            if (!ts.hasTop()) {
                ...
                Key startKey = new Key(jumpKey.getRow(), ts.dataLocation, new Text(ts.term + NULL_BYTE + jumpKey.getColumnFamily()));
                Key endKey = null;
        ...
        public void setNew() {
            if (!this.atEnd && this.iter.hasTop()) {
                ...
                if (this.term == null) {
                    ...
                    int idx = cqString.indexOf(NULL_BYTE);
