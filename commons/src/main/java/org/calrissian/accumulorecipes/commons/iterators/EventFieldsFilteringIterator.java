...
public class EventFieldsFilteringIterator extends Filter {
    ...
    public static void setSelectFields(IteratorSetting is, Set<String> selectFields) {
        is.addOption(SELECT_FIELDS, StringUtils.join(selectFields, DELIM));
    }

    @Override
    public boolean accept(Key k, Value v) {
        if (!k.getColumnFamily().toString().startsWith("fi")) {
            int ifx = k.getColumnQualifier().toString().indexOf(DELIM);
    ...
    public void init(SortedKeyValueIterator<Key, Value> source, Map<String, String> options, IteratorEnvironment env) throws IOException {
        ...
        selectFields = new HashSet<String>(asList(splitPreserveAllTokens(eventFieldsOpt, DELIM)));
    }
